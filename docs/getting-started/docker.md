...
For release reproducibility, this guide uses the pinned image tag `allisson/secrets:v0.3.0`.
...
docker run --rm --name secrets-api --network secrets-net --env-file .env -p 8080:8080 \
  allisson/secrets:v0.3.0 server
```
...
```bash
docker run --rm --network secrets-net --env-file .env allisson/secrets:v0.3.0 create-client \
