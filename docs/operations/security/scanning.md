...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
  aquasec/trivy image allisson/secrets:v0.13.0
...
trivy image --severity HIGH,CRITICAL allisson/secrets:v0.13.0

# Expected output for v0.10.0 (distroless base):
# allisson/secrets:v0.13.0 (debian 13)
...
```bash
# Scan image
trivy image allisson/secrets:v0.13.0

# Filter by severity
trivy image --severity HIGH,CRITICAL allisson/secrets:v0.13.0

# Output formats
trivy image --format json -o results.json allisson/secrets:v0.13.0
trivy image --format sarif -o results.sarif allisson/secrets:v0.13.0  # GitHub Security tab
trivy image --format table allisson/secrets:v0.13.0  # Human-readable table

# Scan specific platforms
trivy image --platform linux/amd64 allisson/secrets:v0.13.0
trivy image --platform linux/arm64 allisson/secrets:v0.13.0

# Exit with error if vulnerabilities found (CI/CD)
trivy image --severity HIGH,CRITICAL --exit-code 1 allisson/secrets:v0.13.0
...
trivy image --ignore-unfixed allisson/secrets:v0.13.0

# Scan with custom policy (fail on specific CVEs)
trivy image --severity HIGH,CRITICAL \
  --ignore-policy .trivyignore \
  allisson/secrets:v0.13.0

# Scan offline (air-gapped environments)
trivy image --download-db-only  # Download vulnerability database
trivy image --skip-update allisson/secrets:v0.13.0  # Scan without updating DB

# Generate SBOM
trivy image --format cyclonedx -o sbom.json allisson/secrets:v0.13.0
trivy image --format spdx-json -o sbom-spdx.json allisson/secrets:v0.13.0
...
trivy image --ignore-policy .trivyignore allisson/secrets:v0.13.0
...
```bash
# Quick scan
docker scout cves allisson/secrets:v0.13.0

# Compare with previous version
docker scout compare --to allisson/secrets:v0.9.0 allisson/secrets:v0.13.0

# Get recommendations
docker scout recommendations allisson/secrets:v0.13.0

# Generate SBOM
docker scout sbom allisson/secrets:v0.13.0 --format cyclonedx > sbom.json

# Policy evaluation (requires Docker Scout subscription)
docker scout policy allisson/secrets:v0.13.0
...
# Docker
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
  anchore/grype:latest allisson/secrets:v0.13.0
...
```bash
# Scan image
grype allisson/secrets:v0.13.0

# Filter by severity
grype allisson/secrets:v0.13.0 --fail-on high

# Output formats
grype allisson/secrets:v0.13.0 -o json > results.json
grype allisson/secrets:v0.13.0 -o sarif > results.sarif

# Generate SBOM with Syft (Anchore's SBOM tool)
syft allisson/secrets:v0.13.0 -o cyclonedx-json > sbom.json
...
```bash
# Scan image
snyk container test allisson/secrets:v0.13.0

# Monitor image (continuous scanning)
snyk container monitor allisson/secrets:v0.13.0

# Scan with custom Dockerfile
snyk container test allisson/secrets:v0.13.0 --file=Dockerfile
...
```bash
# Use clairctl CLI
clairctl report allisson/secrets:v0.13.0
...
trivy image --format cyclonedx -o sbom-cyclonedx.json allisson/secrets:v0.13.0

# SPDX format (recommended for compliance)
trivy image --format spdx-json -o sbom-spdx.json allisson/secrets:v0.13.0

# Human-readable SBOM
trivy image --format json -o sbom-full.json allisson/secrets:v0.13.0
...
# Generate SBOM
syft allisson/secrets:v0.13.0 -o cyclonedx-json > sbom.json
syft allisson/secrets:v0.13.0 -o spdx-json > sbom-spdx.json
syft allisson/secrets:v0.13.0 -o table  # Human-readable
...
oras attach --artifact-type application/vnd.cyclonedx+json \
  allisson/secrets:v0.13.0 sbom.json

# Upload to registry
docker scout sbom allisson/secrets:v0.13.0 --output sbom.json
...
trivy image --format cyclonedx -o sbom-v0.10.0.json allisson/secrets:v0.13.0
...
```bash
# Increase timeout
trivy image --timeout 10m allisson/secrets:v0.13.0

# Use local cache
trivy image --cache-dir /tmp/trivy-cache allisson/secrets:v0.13.0
...
- [Container Security Guide](../security/../deployment/docker-hardened.md) - Runtime security best practices

- [Base Image Migration Guide](../deployment/docker-hardened.md) - Migrate to distroless for fewer CVEs
