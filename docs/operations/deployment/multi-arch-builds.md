...
#   allisson/secrets:v0.13.0 (multi-arch manifest)
...
docker manifest inspect allisson/secrets:v0.13.0
...
...
Multi-platform images pushed: allisson/secrets:latest and allisson/secrets:v0.13.0
...
docker push allisson/secrets:v0.13.0
...
docker pull allisson/secrets:v0.13.0

# On ARM64 host: pulls arm64 image
docker pull allisson/secrets:v0.13.0

# Force pull specific platform (regardless of host)
docker pull --platform linux/arm64 allisson/secrets:v0.13.0
...
docker run --rm allisson/secrets:v0.13.0 uname -m
# x86_64

# Run ARM image on x86_64 host (QEMU emulation)
docker run --rm --platform linux/arm64 allisson/secrets:v0.13.0 uname -m
# aarch64

# Verify application works on both platforms
docker run --rm --platform linux/amd64 allisson/secrets:v0.13.0 --version
docker run --rm --platform linux/arm64 allisson/secrets:v0.13.0 --version
...
# Compare sizes
docker images allisson/secrets:v0.13.0
...
docker buildx build --platform linux/amd64,linux/arm64 --push -t allisson/secrets:v0.13.0 .
...
2. **Use native builders** (build each platform on native hardware):
   ...
   ```bash
   # On x86_64 host
   docker buildx build --platform linux/amd64 --push -t allisson/secrets:v0.13.0-amd64 .
   
   # On ARM64 host
   docker buildx build --platform linux/arm64 --push -t allisson/secrets:v0.13.0-arm64 .
   
   # Create manifest manually
   docker manifest create allisson/secrets:v0.13.0 \
     allisson/secrets:v0.13.0-amd64 \
     allisson/secrets:v0.13.0-arm64
   docker manifest push allisson/secrets:v0.13.0
   ```
...
services:
  secrets:
    image: allisson/secrets:v0.13.0  # Pulls amd64 on x86_64, arm64 on ARM64
    ports:
      - "8080:8080"
