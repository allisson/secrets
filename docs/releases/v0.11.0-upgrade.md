...
v0.11.0 adds account lockout (PCI DSS 8.3.4) to `POST /v1/token`.
...
- [ ] Review `LOCKOUT_MAX_ATTEMPTS` and `LOCKOUT_DURATION_MINUTES` defaults (10 and 30 — satisfy PCI DSS 8.3.4)
...
The defaults satisfy PCI DSS 8.3.4. Override only if your policy requires stricter settings:
