...
v0.9.0 adds cryptographic audit log signing for tamper detection and compliance (PCI DSS Requirement 10.2.2).

**Key Changes:**

- ✅ Automatic HMAC-SHA256 signing of audit logs using KEK-derived signing keys
- ✅ New CLI command: `verify-audit-logs` for integrity verification
- ⚠️ **Breaking:** Database migration 000003 (signature columns + FK constraints)
- ⚠️ **Breaking:** FK constraints prevent deletion of clients/KEKs with audit logs
...
1. ✅ Current version: v0.8.0
2. ✅ Database backup created
3. ✅ KEK chain loaded (check: `curl http://localhost:8080/ready`)
4. ✅ No orphaned client references (see pre-migration checks)
...
⚠️ **WARNING:** Rollback deletes all signature data. Only rollback if absolutely necessary.
