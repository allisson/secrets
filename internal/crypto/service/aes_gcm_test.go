...
func TestNewAESGCM(t *testing.T) {
	...
	t.Run("invalid key size - way too short", func(t *testing.T) {
		key := make([]byte, 15)
		_, err := rand.Read(key)
		require.NoError(t, err)
	...
	t.Run("invalid key size - empty", func(t *testing.T) {
		key := make([]byte, 0)
...
func TestAESGCMCipher_Encrypt(t *testing.T) {
	key := make([]byte, 32)
	_, err := rand.Read(key)
	require.NoError(t, err)
	...
	t.Run("encrypt with plaintext and AAD", func(t *testing.T) {
		...
		assert.Equal(t, 12, len(nonce)) // GCM standard nonce size
	})
...
func TestAESGCMCipher_Decrypt(t *testing.T) {
	key := make([]byte, 32)
	_, err := rand.Read(key)
	require.NoError(t, err)
...
func TestAESGCMCipher_EncryptDecrypt_Integration(t *testing.T) {
	key := make([]byte, 32)
	_, err := rand.Read(key)
	require.NoError(t, err)
