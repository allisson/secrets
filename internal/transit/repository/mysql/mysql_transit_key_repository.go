...
func (m *MySQLTransitKeyRepository) HardDelete(
	...
	if dryRun {
		query := `SELECT COUNT(*) FROM transit_keys WHERE deleted_at IS NOT NULL AND deleted_at < ?`
		var count int64
	...
	query := `DELETE FROM transit_keys WHERE deleted_at IS NOT NULL AND deleted_at < ?`
