...
// Note: TestPostgreSQLSecretRepository_GetByPath_MultipleVersions_LatestDeleted was removed
...
// Note: TestPostgreSQLSecretRepository_GetByPathAndVersion_MultipleVersions_OneDeleted was removed
// because Delete now deletes ALL versions by path (Issue #2), not individual versions.
...
func TestPostgreSQLSecretRepository_ListCursor_FirstPage(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
...
func TestPostgreSQLSecretRepository_ListCursor_SubsequentPages(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
...
func TestPostgreSQLSecretRepository_ListCursor_EmptyResult(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
