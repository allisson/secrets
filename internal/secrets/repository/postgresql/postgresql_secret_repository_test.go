...
func TestPostgreSQLSecretRepository_HardDelete_Success(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
	...
	var countResult int
	err = db.QueryRowContext(ctx, "SELECT COUNT(*) FROM secrets WHERE path = $1", oldSecret.Path).Scan(&countResult)
	require.NoError(t, err)
	assert.Equal(t, 0, countResult)

	// Verify recent deleted secret still exists
	err = db.QueryRowContext(ctx, "SELECT COUNT(*) FROM secrets WHERE path = $1", recentSecret.Path).Scan(&countResult)
	require.NoError(t, err)
	assert.Equal(t, 1, countResult)

	// Verify active secret still exists
	err = db.QueryRowContext(ctx, "SELECT COUNT(*) FROM secrets WHERE path = $1", activeSecret.Path).Scan(&countResult)
	require.NoError(t, err)
	assert.Equal(t, 1, countResult)
}

func TestPostgreSQLSecretRepository_HardDelete_DryRun(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
	...
	var countResult int
	err = db.QueryRowContext(ctx, "SELECT COUNT(*) FROM secrets WHERE path = $1", secret.Path).Scan(&countResult)
	require.NoError(t, err)
	assert.Equal(t, 1, countResult)
}

func TestPostgreSQLSecretRepository_HardDelete_OnlyDeletesSoftDeleted(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
...
func TestPostgreSQLSecretRepository_HardDelete_RespectsTimeThreshold(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
...
func TestPostgreSQLSecretRepository_HardDelete_EmptyResult(t *testing.T) {
	db := testutil.SetupPostgresDB(t)
	defer testutil.TeardownDB(t, db)
	defer testutil.CleanupPostgresDB(t, db)
